...
// code from https://github.com/sravi-uwmadison/prida

#include <opencv2/opencv.hpp>

namespace DECONV_BLIND
{
    struct ctf_params_t
    {
     ...
     *
     **********************************************************************************/
    void gradTVcc(const cv::Mat& f, cv::Mat& dest, int channel)
    {
     ...
     **********************************************************************************/
    void prida(cv::Mat& f, cv::Mat& u, cv::Mat& k, const double lambda, struct params_t params, int channel)
    {
        cv::Mat gradu = cv::Mat::zeros(cv::Size(f.cols + (int)params.NK - 1, f.rows + (int)params.MK - 1), CV_64FC3);
        cv::Mat gradk = cv::Mat::zeros(k.size(), CV_64F);

        for (int i = 0; i < params.niters; i++)
        {
            gradu = cv::Mat::zeros(cv::Size(f.cols + (int)params.NK - 1, f.rows + (int)params.MK - 1), CV_64FC3);
            int c = 0;
            if (channel == 1)
            {
    ...
    struct output buildPyramid(struct input& data, struct output& answer)
    {
        ...
        while (mkpnext > smallestScale && nkpnext > smallestScale &&
               lamnext * data.lambdaMultiplier < data.largestLambda)
        {
            ...
            if (fmod(mkpnext, 2) == 0) mkpnext = mkpnext - 1;

            if (fmod(nkpnext, 2) == 0) nkpnext = nkpnext - 1;
        ...
        for (int s = 1; s < scales; s++)
        {
            ...
            // Makes kernel dimension odd
            if (fmod(answer.MKp[s], 2) == 0) answer.MKp[s] = answer.MKp[s] - 1;

            if (fmod(answer.NKp[s], 2) == 0) answer.NKp[s] -= 1;
            ...
            if (fmod(answer.Mp[s], 2) == 0) answer.Mp[s] -= 1;

            if (fmod(answer.Np[s], 2) == 0) answer.Np[s] -= 1;
     ...
     **********************************************************************************************/
    void coarseToFine(cv::Mat f, struct params_t blind_params, struct ctf_params_t params, struct uk_t& uk, int channel)
    {
        ...
        for (int i = answer.scales - 1; i >= 0; i--)
        {
            ...
            prida(fs, u, k, lambda, blind_params, channel);
            std::cout << "Working on Scale: " << i + 1 << " with lambda = " << data.lambda
                      << " with pyramid_lambda = " << lambda << " and Kernel size " << MKs << std::endl;
        }
        uk.u = u;
        uk.k = k;
    }
     ...
     * Call coarseToFine
     *********************************************************************************************/
    void blind_deconv(cv::Mat f, double lambda, struct params_t& params, struct uk_t& uk, int channel)
    {
        f.convertTo(f, CV_64F, 1. / 255.);
        cv::Mat f3;
        int rpad = 0;
        int cpad = 0;
        if (fmod(f.rows, 2) == 0) rpad = 1;
        if (fmod(f.cols, 2) == 0) cpad = 1;
        ...
        coarseToFine(f3, params, ctf_params, uk, channel);
    }
    ...
    void blind_deconv_helper(cv::Mat image, double LAMBDA, int KERNEL_SIZE, std::string PATH)
    {
        int channel = 1;
        ...
        blind_deconv(image, LAMBDA, params, uk, channel);
